...
int main(int argc, char **argv)
{
    ...
    bool m3dfor = false, mvmfor = false, oneout = false, plyfor = false, offformat = true;

    // sharp quality tolerance
    // value >1 = disabled (default, no argument was given)
    float sq_tol = 1.1;
    ...
    for (unsigned int i = 1; i < argc; i++)
    {
        ...
        case 't':
            sq_tol = atof(argv[i + 1]);
            cout << "Sharp quality tolerance set: " << sq_tol << "\n";
            i++;
            break;
    ...
    if (!octant_start)
    {
        output = mesher.generateMesh(inputs.at(0), ref_level, out_name, all_regions, sq_tol);
    }
    else
    {
