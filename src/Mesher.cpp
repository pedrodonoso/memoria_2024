...
#include <algorithm>
vector<Point3D> octpoints; // puntos con qualityTol
...
namespace Clobscode
{
    ...
    FEMesh Mesher::generateMesh(TriMesh &input, const unsigned short &rl,
                                const string &name, list<RefinementRegion *> &all_reg, float sq_tol)
    {
    ...
    void Mesher::applySurfacePatterns(TriMesh &input)
    {
        ...
        // std::cout << "Octants skipped: " << passescounter << "\n";
    ...
    void Mesher::elementsJENS(float sq_tol)
    {

        // vector con elementos outside
        vector<Element *> octelements;
        // only hexahedra are used
        for (auto i : octants)
        {
            if (i.isInside())
            {
                continue;
            } // intersected_faces.empty()
            for (auto j : i.getSubElements())
            { // intersected_faces.notEmpty()
                if (j.size() == 8)
                {
        ...
        Quality quality;
        quality.allJENS_sharp_points(octpoints, octelements, sq_tol);

        // std::cout << "Total elements including inside oct: " << octants.size() << "\n";
        // std::cout << "Size octelements vector (external): " << octelements.size() << "\n";
    }
}
